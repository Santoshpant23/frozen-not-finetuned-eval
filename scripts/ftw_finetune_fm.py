"""End-to-end fine-tuning of a GeoFM (Prithvi/TerraMind) for dense FTW segmentation.
Robust recipe (BCE+Dice, capped pw, warmup). --frac/--seed/--freeze/--tag, and
--eval-country (CROSS-REGION: train on positional country, eval on --eval-country test pixels)."""
...
def run(fm, C):
    ...
    order = list(range(len(train_chips)))
    ...
    y = test.label.to_numpy(); auc = round(roc_auc_score(y, probs), 4); f1 = round(f1_score(y, (probs >= .5).astype(int)), 4)
    print(f"{C}->{ec} [{a.model}-ft seed={a.seed} freeze={a.freeze}] AUROC={auc} F1={f1} n={len(test)}\n", flush=True)
    return {"train_country": C, "eval_country": ec, "ft_true_auroc": auc, "ft_true_f1": f1, "n_eval": int(len(test)), "frac": a.frac, "seed": a.seed, "freeze": a.freeze}
OUTF = "data/results/ftw_finetune_fm_" + a.model + a.tag + ".json"
out = {}; fm = make_fm()
for C in COUNTRIES: out[C] = run(fm, C); json.dump(out, open(OUTF, "w"), indent=2)
print("wrote " + OUTF)
