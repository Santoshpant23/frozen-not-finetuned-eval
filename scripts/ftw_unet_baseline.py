"""Supervised U-Net baseline. --robust (BCE+Dice,capped pw,warmup), --frac (low-data,
seed-deterministic), --seed, --perpixel (1x1 ablation), --eval-country (CROSS-REGION:
train on positional country, evaluate on --eval-country's canonical test pixels)."""
...
dev = "cuda" if torch.cuda.is_available() else "cpu"; EP = 150
...
def run(C):
    ...
    print(f"{C}->{ec} [{tag} seed={a.seed}] AUROC={auc} F1={f1} n={len(test)}\n", flush=True)
    return {"train_country": C, "eval_country": ec, "unet_true_auroc": auc, "unet_true_f1": f1, "n_eval": int(len(test)), "frac": a.frac, "seed": a.seed}
