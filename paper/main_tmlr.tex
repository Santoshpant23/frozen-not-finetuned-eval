...
exceeds full fine-tuning, in-region for Prithvi-EO-2.0 (six regions) and TerraMind
(India and France), and cross-region for Prithvi-EO-2.0, while training about
$110\times$ fewer parameters. Fine-tuning also
...
mostly reflects the weak per-pixel baseline. ``frozen FM'' is the per-region best
frozen GeoFM with a linear probe across Prithvi-EO-2.0, TerraMind, Clay, and AnySat;
superscripts identify the winner per row ($^{\textsc{p}}$ = Prithvi-EO-2.0,
$^{\textsc{a}}$ = AnySat). ``FT FM'' = fine-tuned Prithvi. U-Net and FT FM are
mean$\pm$std over 3 seeds; RF and the linear probe are deterministic given the split.
The apples-to-apples frozen-backbone-plus-decoder comparison in Table~\ref{tab:spine}
uses Prithvi only; per-model tile-disjoint sensitivity is in
Appendix Table~\ref{tab:tilesplit}.}
\label{tab:flaws}
\begin{tabular}{lccccc}
\toprule
Region & RF \textsc{true} & RF \textsc{proxy} & U-Net & frozen FM & FT FM \\
\midrule
India        & 0.574 & 0.786 & 0.909\,{\scriptsize$\pm$.034} & \textbf{0.983}$^{\textsc{p}}$ & 0.983\,{\scriptsize$\pm$.003} \\
Cambodia     & 0.550 & 0.900 & 0.948\,{\scriptsize$\pm$.001} & 0.924$^{\textsc{p}}$ & 0.949\,{\scriptsize$\pm$.000} \\
Vietnam      & 0.643 & 0.911 & 0.962\,{\scriptsize$\pm$.003} & 0.929$^{\textsc{p}}$ & 0.951\,{\scriptsize$\pm$.000} \\
Kenya        & 0.651 & 0.806 & 0.887\,{\scriptsize$\pm$.002} & 0.853$^{\textsc{a}}$ & 0.763\,{\scriptsize$\pm$.012} \\
France       & 0.695 & 0.959 & 0.977\,{\scriptsize$\pm$.000} & 0.961$^{\textsc{p}}$ & 0.978\,{\scriptsize$\pm$.001} \\
Netherlands  & 0.815 & 0.928 & 0.981\,{\scriptsize$\pm$.002} & 0.961$^{\textsc{a}}$ & 0.959\,{\scriptsize$\pm$.005} \\
\bottomrule
\end{tabular}
\end{table}
...
performance matches full fine-tuning on five of six regions for Prithvi-EO-2.0
(within $\pm0.013$ AUROC); TerraMind was tested in two regions (frozen-vs-fine-tune
deltas of $+0.001$ in India and $+0.006$ in France). Fine-tuning the 300M-parameter
backbone gives no in-region gain. An
...
which are cheap, stable, and transfer-competitive in that configuration rather than
as backbones to fine-tune.
...
\begin{table}[h]\centering\small
\caption{\textbf{Tile-disjoint sensitivity of the Table~\ref{tab:flaws}
frozen-probe column (true-label AUROC).} Each pair gives chip-grouped (``chip'',
used in the main text) vs.\ MGRS-tile-disjoint (``tile'') AUROC for the same frozen
linear probe. Prithvi-EO-2.0 and TerraMind are stable across both splits in every
tested region (drops $\le 0.01$). AnySat scores collapse on Kenya
($0.853 \to 0.580$), Vietnam ($0.897 \to 0.645$), and France ($0.943 \to 0.605$),
indicating that AnySat's chip-grouped advantage may not fully reflect transferable
feature quality. In Kenya, where Table~\ref{tab:flaws} reports AnySat $0.853$ as the
best probe, Prithvi at $0.761$ is more robust under tile-disjoint than AnySat at
$0.580$. Cambodia is omitted because the tile-disjoint variant was not run. We use
chip-grouped numbers in the main text because they preserve the per-chip statistical
assumptions of the controlled comparison; this table is the sensitivity check.}
\label{tab:tilesplit}
...
In the extreme-sparsity regime the supervised CNN wins on every
metric, not only AUROC. ``Frozen'' = Prithvi backbone
